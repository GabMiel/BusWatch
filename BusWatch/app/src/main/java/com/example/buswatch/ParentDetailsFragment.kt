...
class ParentDetailsFragment : Fragment() {
        ...
        db.collection("parents").document(uid).get()
            .addOnSuccessListener { document ->
                ...
                if (document != null && document.exists()) {
                    ...
                    val fName = profile?.get("firstName") as? String ?: document.getString("firstName") ?: ""
                    val lName = profile?.get("lastName") as? String ?: document.getString("lastName") ?: ""
                    ...
                    view.findViewById<TextView>(R.id.textView48).text = profile?.get("email") as? String ?: document.getString("email") ?: ""
                    view.findViewById<TextView>(R.id.textView50).text = profile?.get("phone") as? String ?: document.getString("phone") ?: ""
                    
                    val parentAvatarUrl = profile?.get("avatarUrl") as? String ?: document.getString("avatarUrl")
    ...
    private fun showEditProfileDialog() {
        ...
        db.collection("parents").document(uid).get().addOnSuccessListener { doc ->
            ...
            etFirstName.setText(profile?.get("firstName") as? String ?: doc.getString("firstName"))
            etLastName.setText(profile?.get("lastName") as? String ?: doc.getString("lastName"))
            etMiddleName.setText(profile?.get("middleName") as? String ?: doc.getString("middleName"))
            
            selectedSuffix = profile?.get("suffix") as? String ?: doc.getString("suffix") ?: ""
            ...
            etEmail.setText(profile?.get("email") as? String ?: doc.getString("email"))
            etPhone.setText(profile?.get("phone") as? String ?: doc.getString("phone"))
            
            val avatarUrl = profile?.get("avatarUrl") as? String ?: doc.getString("avatarUrl")
    ...
    private fun uploadParentAvatar(uri: Uri, updates: MutableMap<String, Any>, dialog: AlertDialog) {
        val uid = auth.currentUser?.uid ?: return
        ...
        ref.putFile(uri)
            ...
            .addOnSuccessListener { downloadUrl ->
                updates["profile.avatarUrl"] = downloadUrl.toString()
                // Also update root avatarUrl for backward compatibility
                updates["avatarUrl"] = downloadUrl.toString()
